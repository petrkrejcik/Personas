...
const getPersons = () => {
    const persons = Object.values(getState().persons)
        ...
        return Object.assign(Object.assign({}, person), { age: getAge(person.birthday), daysToBirthday: getDaysToBirthday(person.birthday), seenBefore: getSeenBefore(person.seen), onEditClick: id => {
            ...
            }, onRemoveClick: toggleRemoveOverlay, cancelRemove: toggleRemoveOverlay, remove: remove, save: save.bind(null, person.id) });
    })
        .sort(sortByBirthday);
    return persons;
};
...
const save = (id) => {
    ...
    const person = Object.assign(Object.assign({ id: id || createId(personRest.name) }, personRest), { birthday: createIso(day, month, year) });
