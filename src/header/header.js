...
export default function render() {
	...
	const content = [renderTitle(), spaceEl, renderSync(), renderAdd()];
...
const renderSync = () => {
	...
	if (!isSyncOk()) {
		...
		warn = div(
			{ className: "header-button--warn", testId: `header-button--warn` },
			ICONS.warn
		);
	}
	...
	return div({ className: "header__syncWrap" }, [
		div(
			{ className: classNames, testId: `header-button--sync`, onClick },
			ICONS.sync
		),
		warn,
	]);
